...
export default function App() {
  ...
  const handleAddStoreAI = async () => {
    ...
    const prompt = `作为专业的台湾旅游达人，请根据你的知识库为我介绍这个目标（它可能是一家店面、一个景点、一款伴手礼产品或特色美食）："${storeTarget}"。
请了解清楚它的核心特色、评价与实用信息后，严格按照以下 JSON 格式输出结果。不要输出任何其他多余的文字或 markdown 标记（例如不要输出 \`\`\`json）：
{
  "n": "${newStoreName || '名称（店名/地名/产品名）'}",
  "f": "特色、背景与亮点简述",
  "do": "必做体验或主要用途（一句话）",
  "eat": "必吃/必买/搭配推荐（逗号分隔，如果不适用可填无）",
  "w": "避雷指南或注意事项（如果有）",
  "r": "网络评价摘要",
  "tips": "实用提示（一句话）",
  "price": "消费预估或产品价格",
  "minSpend": "低消限制或购买门槛（如果不适用可填无）",
  "zone": "所属区域（例如台北信义区、或者购买渠道/商超名称）",
  "cuisine": "分类类型（如：小吃、景点、护肤品、伴手礼等）",
  "hours": "营业时间（如果不适用可填无）",
  "lat": "精准纬度(数字，例如 25.033。若为具体地点请尽全力估算；若是实体产品且无特定专卖店可填 0)",
  "lng": "精准经度(数字，例如 121.564。若是实体产品且无特定专卖店可填 0)"
}`;
...
[
  ...
  const handleCopyPromptOnly = () => {
    const storeTarget = searchQuery || "值得推荐的地点或好物";
    const prompt = `作为专业的台湾旅游达人，请根据你的知识库为我介绍这个目标（它可能是一家店面、一个景点、一款伴手礼产品或特色美食）："${storeTarget}"。
请了解清楚它的核心特色、评价与实用信息后，严格按照以下 JSON 格式输出结果。不要输出任何其他多余的文字或 markdown 标记（例如不要输出 \`\`\`json）：
{
  "n": "名称（店名/地名/产品名）",
