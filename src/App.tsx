...
export default function App() {
  ...
  const [promptType, setPromptType] = useState<'standard' | 'hardcore'>(
    (localStorage.getItem('PREFERRED_PROMPT_TYPE') as 'standard' | 'hardcore') || 'standard'
  );
                  ...
                  searchQuery.trim() !== '' ? (
                    <div className="flex flex-col items-center justify-center p-8 mt-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        <button 
                          onClick={() => {
                            const prompt = getStandardPrompt(searchQuery);
                            navigator.clipboard.writeText(prompt).then(() => {
                              toast.success('常规版 Prompt 已复制，请前往 AI 粘贴');
                            }).catch(() => toast.error('请手动复制 Prompt'));
                          }}
                          className="bg-[#2D3436] hover:bg-black text-white px-5 py-3 rounded-xl font-bold flex items-center gap-2 transition-colors text-sm"
                        >
                          ✨ 常规详细版
                        </button>
                        <button 
                          onClick={() => {
                            const prompt = getHardcorePrompt(searchQuery);
                            navigator.clipboard.writeText(prompt).then(() => {
                              toast.success('尽调版 Prompt 已复制，请前往 AI 粘贴');
                            }).catch(() => toast.error('请手动复制 Prompt'));
                          }}
                          className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-3 rounded-xl font-bold flex items-center gap-2 transition-colors shadow-sm text-sm"
                        >
                          🔥 硬核尽调版
                        </button>
                      </div>
                        ...
                        <div className="flex items-center justify-between mt-5 border-t border-gray-100 pt-4">
                          ...
                          <div className="flex items-center gap-2">
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                const prompt = getStandardPrompt(loc.n);
                                navigator.clipboard.writeText(prompt).then(() => {
                                  toast.success('常规版 Prompt 已复制，请前往 AI 粘贴');
                                }).catch(() => toast.error('请手动复制 Prompt'));
                              }}
                            ...
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                const prompt = getHardcorePrompt(loc.n);
                                navigator.clipboard.writeText(prompt).then(() => {
                                  toast.success('尽调版 Prompt 已复制，请前往 AI 粘贴');
                                }).catch(() => toast.error('请手动复制 Prompt'));
                              }}
                              className="w-9 h-9 rounded-full flex items-center justify-center transition-all bg-orange-50 hover:bg-orange-100 text-orange-500 border border-orange-200"
                              title="问 AI 这个店的详情 (硬核尽调版)"
                            >
                    ...
                    <div className="loc-card bg-indigo-50/50 p-6 flex flex-col items-center justify-center text-center transition-all duration-300 border-2 border-dashed border-indigo-200 rounded-2xl hover:bg-indigo-50">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        <button 
                          ...
                          className="bg-[#2D3436] hover:bg-black text-white px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors shadow-sm cursor-pointer"
                        >
                          <Sparkles className="w-4 h-4" /> 常规详细版
                        </button>
                        <button 
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="bg-indigo-500 hover:bg-indigo-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors shadow-sm cursor-pointer"
                        >
                          🔥 硬核尽调版
                        </button>
                      </div>
